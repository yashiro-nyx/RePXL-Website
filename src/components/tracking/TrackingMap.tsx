...
/**
 * TrackingMap — contextual delivery route map using react-leaflet + OpenStreetMap.
 *
 * Only renders when status is "Out for Delivery" or "Delivered".
 * Shows a Polyline route and a vehicle marker that moves along it based on progress.
 *
 * react-leaflet requires a browser environment — loaded via next/dynamic (ssr: false)
 * by the parent to avoid SSR hydration issues with Leaflet's window references.
 */
...
// ── Coordinate route: distribution hub → customer location ───────────────────
// A realistic Metro Manila delivery route (warehouse in Parañaque → Quezon City)
const ROUTE_COORDS: [number, number][] = [
  [14.4793, 121.0198], // Parañaque distribution hub
  [14.4870, 121.0180],
  [14.5020, 121.0155],
  [14.5200, 121.0120],
  [14.5430, 121.0198],
  ...
  [14.6200, 121.0400],
  [14.6420, 121.0450],
  [14.6760, 121.0440], // Quezon City delivery address
]
...
// Interpolate position along the route based on 0–1 progress fraction
...
// Helper component to smoothly re-center the map when marker moves
...
export function TrackingMap({ status, progress }: TrackingMapProps) {
  ...
  const visible = status === 'Out for Delivery' || status === 'Delivered'
  ...
  return (
    <div className="mt-5 overflow-hidden rounded-2xl border border-repixl-muted/10">
      <div className="flex items-center justify-between bg-repixl-charcoal px-4 py-3">
        ...
        <p className="font-mono text-[9px] text-repixl-muted/60">OpenStreetMap</p>
      </div>
