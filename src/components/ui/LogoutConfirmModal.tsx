...
export function LogoutConfirmModal({ isOpen, onCancel, onConfirm }: LogoutConfirmModalProps) {
  ...
  useEffect(() => {
    ...
    // Focus the Cancel button by default so the safe action is immediately reachable
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Tab') {
        const focusables = overlay.querySelectorAll<HTMLElement>('button, [tabindex]:not([tabindex="-1"])')
    ...
    >
      ...
      <div className="relative w-full max-w-sm rounded-lg border border-repixl-muted/20 bg-repixl-bg p-6 shadow-2xl">
        ...
        <button
          ...
          className="absolute right-3 top-3 flex h-7 w-7 items-center justify-center rounded-full text-repixl-muted hover:bg-repixl-charcoal hover:text-repixl-text-light transition-colors"
        >
        ...
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-repixl-charcoal">
          <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-repixl-red" aria-hidden="true">
        ...
        <div className="mt-4 text-center">
          <h2 id="logout-modal-title" className="font-display text-lg font-semibold text-repixl-text-light">
            Log out?
          </h2>
